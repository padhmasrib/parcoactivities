...
void random_init(simulation& s) {
  ...
  for (size_t i = 0; i < s.nbpart; ++i) {
    ...
    s.x[i] = dispos(gen);
    s.y[i] = dispos(gen);
    // s.z[i] = dispos(gen); // is it okay to set z = 0.0?
    s.z[i] = 0.0;

    s.vx[i] = disvel(gen);
    s.vy[i] = disvel(gen);
    // s.vz[i] = disvel(gen); // is it okay to set z vel to 0.0?
    // s.vx[i] = s.y[i]*1.5;
    // s.vy[i] = -s.x[i]*1.5;
  }
...
void update_force(simulation& s, size_t from, size_t to, double& fx, double& fy, double& fz) {
  double softening = .1;
  ...
  // apply force - collect them in thread local vars from the caller
  fx += dx * F;
  fy += dy * F;
  fz += dz * F;
}
...
int main(int argc, char* argv[]) {
  ...
  for (size_t step = 0; step < nbstep; step++) {
    ...
    #pragma omp parallel
    {
      // using thread local vectors for accumulation of forces
      std::vector<double> fx_TL(s.nbpart, 0.0);
      std::vector<double> fy_TL(s.nbpart, 0.0);
      std::vector<double> fz_TL(s.nbpart, 0.0);
      ...
      for (size_t i = 0; i < s.nbpart; ++i) {
        for (size_t j = 0; j < s.nbpart; ++j) {
          if (i != j) {
            ...
            update_force(s, i, j, fx_local, fy_local, fz_local);
            // accumulated in thread-local vectors
            fx_TL[j] += fx_local;
            fy_TL[j] += fy_local;
            fz_TL[j] += fz_local;
          }
